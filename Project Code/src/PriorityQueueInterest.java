 ...
 */
public class PriorityQueueInterest<Object> extends ArrayList<Object> {
	private int size;

	public PriorityQueueInterest() {
		size = 0;
	}
	...
	public void percolateDown(int spot) {
		if (spot != super.size() - 1) {
			...
			if (rspot <= super.size() - 1) {
				...
				if (element.getInterest() > lelement.getInterest()
						|| element.getInterest() > relement.getInterest()) {

					if (relement.getInterest() < lelement.getInterest()) {
						super.set(spot, (Object) relement);
						super.set(rspot, (Object) element);
						percolateDown(rspot);
					} else if (relement.getInterest() > lelement.getInterest()) {
	...
	public void percolateUp(int index) {
		...
		if (parent.getInterest() > ((Route) super.get(index)).getInterest()) {
